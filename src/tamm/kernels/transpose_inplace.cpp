...
#ifdef USE_DPCPP
#define __global__ __attribute__((always_inline)) inline
#define __device__ SYCL_EXTERNAL __attribute__((always_inline))
#endif
...
constexpr int THREADS_Y = 256 / TILE_DIM;
  ...
  template<typename T>
  __device__ T doOp(T in, const bool conjugate) {
    //return conjugate ? std::conj(in) : in;
    return in;
  }
  ...
  __global__ void transposeInPlaceKernel(T* in_ptr,
    ...
    if (blockIdx_y > blockIdx_x) { // Off diagonal blocks
      ...
      for (int repeat = 0; repeat < TILE_DIM; repeat += THREADS_Y) {
        int dy_ = dy + repeat;
        if (is32Multiple || (dx < iDim0 && dy_ < iDim1))
          iptr[dy_ * iStride1 + dx] = doOp<T>(shrdMem_s[lx][ly + repeat], conjugate);

        int gy_ = gy + repeat;
        if (is32Multiple || (gx < iDim0 && gy_ < iDim1))
          iptr[gy_ * iStride1 + gx] = doOp<T>(shrdMem_d[lx][ly + repeat], conjugate);
      }
      ...
      for (int repeat = 0; repeat < TILE_DIM; repeat += THREADS_Y) {
        int gy_ = gy + repeat;
        if (is32Multiple || (gx < iDim0 && gy_ < iDim1))
          iptr[gy_ * iStride1 + gx] = doOp<T>(shrdMem_s[lx][ly + repeat], conjugate);
      }
    }
  }

  template<typename T>
  void transpose_inplace(T* out, const T* in, int* outDims, int* inDims, int* rdims,
                         gpuStream_t& thandle) {

    // check for conjugate here (only for complex dataypes)
    bool conjugate = std::is_arithmetic_v<T>;
    ...
    std::cout << "range of kernel launch: (" << global[0] << ", " << global[1] << ", " << global[2] << "), " << local[0] << ", " << local[1] << ", " << local[2] << std::endl;
    ...
    thandle.first.parallel_for(sycl::nd_range<3>(global, local), [=](auto item) {
      transposeInPlaceKernel<T>(out, outDims, outStride, blk_x, blk_y, conjugate, is32Multiple, item);
      });
#endif
  }
  ...
  template void transpose_inplace(double* out, const double* in, int* outDims, int* inDims, int* rdims, gpuStream_t& thandle);
  template void transpose_inplace(float* out, const float* in, int* outDims, int* inDims, int* rdims, gpuStream_t& thandle);
  template void transpose_inplace(std::complex<double>* out, const std::complex<double>* in, int* outDims, int* inDims, int* rdims, gpuStream_t& thandle);
  template void transpose_inplace(std::complex<float>* out, const std::complex<float>* in, int* outDims, int* inDims, int* rdims, gpuStream_t& thandle);
